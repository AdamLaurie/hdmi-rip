...
parser.add_option("-k", "--keyboard", action="store_true", dest="keyboard", default= False, help="start/stop recording under keyboard control (False)")
...
parser.add_option("-S", "--strict", action="store_true", dest="strict", default= False, help="strict mode - abort recording if frames dropped (False)")
parser.add_option("-w", "--wave", action="store_true", dest="wave", default= False, help="save audio in .wav format (False)")
		...
		if (dest_port==2068):
			if options.keyboard:
				if len(select.select([sys.stdin],[],[],0)[0]) != 0:
					...
					if recording:
						log('Recording started at: %s' % datetime.datetime.fromtimestamp(time.time()))
					else:
						log('Recording stopped at %s' % datetime.datetime.fromtimestamp(time.time()))
			...
			if (part == 0):
				if not Videostarted:
					...
					if options.keyboard:
						log('Hit <ENTER> to START/STOP recording')
					log('CTL-C to exit')
					Videostarted= 1
					if record_time:
				...
				if dropping:
					...
					if recording:
						...
						if not Video_Frames % 100 and len(audio_buf) == 0:
							log("*** Warning! No AUDIO! Possible incompatible codec - set source to Stereo/PCM?") 
						if options.wave:
